<template>
  <div class="container mt-4">
    ...
    <table class="table table-bordered">
      <thead>
        <tr>
          <th>S.No</th><th>Req No</th><th>PL No</th><th>User</th><th>Part No</th><th>Status</th><th>Actions</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(r, i) in visibleRequests" :key="r.id">
          ...
          <td>
            ...
            <!-- Edit/Delete Monitoring from table row -->
            ...
            <!-- Edit/Delete Report from table row -->
            ...
            <!-- Complete button: only shows if both saved and not yet completed -->
            <button
            ...
            <button class="btn btn-danger btn-sm tt" data-tip="Delete entire request including Monitoring & Report" @click="confirmDeleteRequest(r)">🗑 Del Req</button>
          </td>
        </tr>
        <tr v-if="visibleRequests.length === 0">
          <td colspan="7" class="text-center text-muted py-3">No active requests. Completed requests are removed from this list.</td>
        </tr>
      </tbody>
    </table>
    ...
    <div v-if="monitoring" class="modal-overlay">
      <div class="modal-box" style="max-height:90vh;overflow-y:auto">
        ...
        <!-- ✅ Monitoring modal action buttons — same pattern as Report -->
        <div class="d-flex gap-2 mt-2 flex-wrap">
          <button class="btn btn-info tt" data-tip="Print as A4 Landscape Datasheet" @click="printMonitoringDirect">🖨 Print Datasheet</button>
          <button class="btn btn-outline-danger tt" data-tip="Download Datasheet as PDF" @click="printMonitoringDirect">⬇ PDF</button>
          <button
            class="btn btn-success tt"
            :data-tip="monitoringEditMode ? 'Update monitoring in database' : 'Save monitoring to database'"
            @click="submitDaily"
          >{{ monitoringEditMode ? 'Update' : 'Save' }}</button>
          <!-- ✅ Edit mode: show Delete button inside modal -->
          <button
            v-if="monitoringEditMode"
            class="btn btn-danger tt"
            data-tip="Permanently delete this monitoring record"
            @click="confirmDeleteMonitoringFromModal"
          >🗑 Delete</button>
          <button class="btn btn-secondary tt" data-tip="Close without saving" @click="monitoring = null">Close</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="report" class="modal-overlay">
      <div class="modal-box large" style="max-height:90vh;overflow-y:auto">
        ...
        <div v-if="reportSaveError" class="alert-error mb-3">
          ...
          <button class="close-err tt" data-tip="Dismiss error" @click="reportSaveError = ''">×</button>
        </div>
        ...
        <div class="section-header mt-3">📘 Monitoring Details (Auto-filled from Monitoring)</div>
        ...
        <label class="form-label">
          Acceptance Criteria
          <span class="badge-autofill tt" data-tip="Pre-filled from request — you may edit freely">auto-filled</span>
        </label>
        ...
        <div v-if="report.failurePhotos && report.failurePhotos.length" class="photo-preview-row mb-2">
          <div v-for="(p, idx) in report.failurePhotos" :key="idx" class="photo-thumb-wrap">
            ...
            <button class="thumb-remove tt" data-tip="Remove photo" @click="removePhoto(idx)">×</button>
          </div>
        </div>
        ...
        <div class="d-flex gap-2 mt-3 flex-wrap">
          <button class="btn btn-primary tt" data-tip="Print Report as A4 Portrait (with signatures & photos)" @click="printReportDirect">🖨 Print Report</button>
          <button class="btn btn-outline-danger tt" data-tip="Download Report as PDF (with signatures & photos)" @click="downloadPDF('report')">⬇ PDF</button>
          <button
            class="btn btn-success tt"
            :data-tip="reportEditMode ? 'Update report in database' : 'Save report to database'"
          ...
          <button v-if="reportEditMode" class="btn btn-danger tt" data-tip="Permanently delete this report" @click="confirmDeleteReport(report.reqNo)">🗑 Delete</button>
          <button class="btn btn-secondary tt" data-tip="Close without saving" @click="closeReport">Close</button>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      monitoringKeys: [],
      reportKeys: [],
      completedKeys: [],   // ✅ tracks which requestNos are completed (to hide from list)
      monitoringCache: {},
      reportCache: {},

      deleteModal: { show: false, type: '', typeLabel: '', requestNo: '', dbId: null, requestObj: null },
  ...
  computed: {
    // ✅ Only show non-completed requests in the Daily Update table
    visibleRequests() {
  ...
  mounted() {
    ...
    this._ttOver = (e) => {
      ...
      if (el) {
        ...
        this.tooltipStyle = { top: (r.bottom + window.scrollY + 7) + 'px', left: Math.max(8, r.left + window.scrollX) + 'px' }
      } else {
        this.tooltipText = ''
      }
    }
  ...
  methods: {
    ...
    openMonitoring(r) {
      ...
      this.monitoring = {
        plNo: r.allocationPlNo||'', requestNo: r.requestNo||'', partNo: r.partNo||'',
        description: r.description||'', customer: r.customer||'',
        date: todayDisplay(),
        testType: r.category||r.testType||'', purpose: r.category||r.testType||'',
        samples: r.samples||'', testDetails: r.testDetails||'',
        equipmentName: '', equipmentNo: '',
        standard: r.standard||r.spec||'',
        requestDate: r.requestDate ? r.requestDate.substring(0,10) : todayISO(),
        testStartedOn: '', testCompletedOn: '',
        targetCycle: '', initialReading: '', currentReading: '',
        remarks: '',
        acceptanceCriteria: r.acceptanceCriteria||r.acceptance||'',
        testResults: '', responsibility: 'Admin', requestedBy: r.userName||'User', dbId: null,
      }
    },
    ...
    // ✅ Called from inside the monitoring modal when in edit mode
    confirmDeleteMonitoringFromModal() {
      ...
      this.monitoring = null  // close the modal first
      this.deleteModal = { show: true, type: 'monitoring', typeLabel: 'Monitoring', requestNo, dbId, requestObj: null }
    },
    ...
    async submitDaily() {
      try {
        ...
        alert('Error saving monitoring: ' + (err.response?.data?.message || err.message))
      }
    },

    confirmDeleteMonitoring(requestNo) {
      const cached = this.monitoringCache[requestNo]
      this.deleteModal = { show: true, type: 'monitoring', typeLabel: 'Monitoring', requestNo, dbId: cached?.dbId||null, requestObj: null }
      this.monitoring = null
    },
    ...
    openReport(r) {
      ...
      this.report = {
        ...
        reqNo: r.requestNo||'', plNo: r.allocationPlNo||'', date: todayDisplay(),
        description: r.description||'', partNo: r.partNo||'',
        customer: r.customer||'', component: r.component||'',
        testName: r.testName||'', special: r.special||'',
        category: r.category||r.testType||'', testType: r.category||r.testType||'',
        samples: r.samples||'', standard: r.standard||r.spec||'',
        testDetails: r.testDetails||'',
        spec: mon.equipmentName||'',
        equipmentName: mon.equipmentName||'', equipmentNo: mon.equipmentNo||'',
        initialReading: mon.initialReading||'', currentReading: mon.currentReading||'', targetCycle: mon.targetCycle||'',
        criteria: r.acceptanceCriteria||r.acceptance||'',
        observation: '', conclusion: '', result: 'Passed',
        reportedBy: '', approvedBy: '', requestedBy: r.userName||'',
        postDataBase64: null, postDataName: null, failurePhotos: [],
        signReportedPreview: null, signApprovedPreview: null, dbId: null,
      }
    },
    ...
    closeReport() { this.report = null; this.reportSaveError = '' },

    confirmDeleteReport(requestNo) {
      const cached = this.reportCache[requestNo]
      this.deleteModal = { show: true, type: 'report', typeLabel: 'Report', requestNo, dbId: cached?.dbId||null, requestObj: null }
      this.report = null
    },

    confirmDeleteRequest(r) {
      this.deleteModal = {
        show: true, type: 'request',
    ...
    // ✅ Complete request — updates status in DB, adds to completedKeys so row disappears from visible list
    async completeRequest(r) {
      ...
      if (!confirm(`Mark Request ${requestNo} as Completed?\nThis will remove it from the Daily Update list.`)) return
      try {
        ...
        // ✅ Add to completedKeys — triggers visibleRequests computed to hide the row
    ...
    async onFailurePhotosChange(e) {
      for (const file of Array.from(e.target.files)) this.report.failurePhotos.push(await fileToBase64(file))
    },
    ...
    async submitReport() {
      ...
      if (!this.report.reqNo) {
        this.reportSaveError = 'Request No is missing. Cannot save report.'
        return
      }
      this.reportSaving = true
      try {
        const reqNo = this.report.reqNo
        const payload = { ...this.report, reportBalance: this.reportBalance }
        ...
        if (this.reportEditMode && existingDbId) {
          ...
          const newId = res.data?.data?.id || res.data?.id || res.data?.insertId || res.data?.report?.id || null
        ...
        this.reportSaveError =
          ...
          err.message ||
          'Unknown server error — check browser console (F12) for details.'
      } finally {
    ...
    printMonitoringDirect() { if (this.monitoring) this._printMonitoring(this.monitoring) },
    printReportDirect() { if (this.report) this._printReport(this.report, this.reportBalance) },
    printSheet(type) {
    ...
    // ✅ Full Report print — includes signatures, failure photos, post data attachment info
    _printReport(d, bal) {
      ...
      const failureBlock = (d.failurePhotos||[]).length
        ? `<hr style="border:none;border-top:1px solid #000;margin:6px 0;"/>
           <div><b>Failure Photos:</b><br/><div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px;">${photoImgs}</div></div>` : ''
...
<div class="body-row">
  <span class="lbl">Product :</span><span class="val">${d.description||''}</span>
...
<div class="footer-line"><span>FORM NO: J21 - 03</span><span>RETENTION PERIOD : 3 YEARS</span></div>
    ...
    // ✅ Monitoring / Datasheet print — A4 Landscape
    _printMonitoring(d) {
      const logo = window.location.origin + '/images/TVS.jpg'
...
.modal-box {
  background: #fff; padding: 24px; width: 540px;
  margin: 36px auto; border-radius: 6px;
  box-shadow: 0 8px 32px rgba(0,0,0,0.25);
}
.large { width: 760px; }
.section-header {
  font-weight: 600; font-size: 13.5px; color: #1a56a0;
  background: #eef4ff; border-left: 4px solid #1a56a0;
...
.alert-error {
  ...
  color: #b91c1c; border-radius: 5px;
...
.file-tag { display: inline-block; background: #e8f5e9; border: 1px solid #a5d6a7; border-radius: 4px; padding: 3px 10px; font-size: 13px; color: #2e7d32; }
...
.sig-preview { display: block; height: 60px; border: 1px dashed #aaa; border-radius: 4px; padding: 3px; object-fit: contain; background: #fafafa; }
.gap-2 { gap: 8px; }
